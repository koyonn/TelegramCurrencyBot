...
public class CurrencyConverterImpl implements CurrencyConverterContract {
    ...
    public Map<String, NBRBCurrency> getCurrencyMap() {
        Map<String, NBRBCurrency> copyOfCurrencyMap = new HashMap<>(currencyMap);
    ...
    public String getCurrencyRatesFull() {
        ...
        set.forEach(e -> {
            long scale = e.getValue().getScale();
            String curName = e.getValue().getCurName();
            double officialRate = e.getValue().getOfficialRate();
            sb.append(
                    String.format("%d %s = %.2f %s",
                            scale, curName, officialRate, Constants.getBYN().getCurName()) + "\n");
        });
        String currencyList = new String(sb);
        return currencyList;
    }
    ...
    public String getCurrencyRatesShort() {
        StringBuilder sb = new StringBuilder();
        Constants.getMainCurrencyAbbreviation().forEach(e -> {
            if (!e.equals("BYN")) {
                long scale = currencyMap.get(e).getScale();
                String curName = currencyMap.get(e).getCurName();
                double officialRate = currencyMap.get(e).getOfficialRate();
                sb.append(
                        String.format("%d %s = %.2f %s",
                                scale, curName, officialRate, Constants.getBYN().getCurName()) + "\n");
            }
    ...
    public String getCurrencyExchangeRates(String originalAbbreviation, String targetAbbreviation, double value) {
        double originalRate = currencyMap.get(originalAbbreviation).getOfficialRate();
        double originalScale = currencyMap.get(originalAbbreviation).getScale();
        double targetRate = currencyMap.get(targetAbbreviation).getOfficialRate();
        double targetScale = currencyMap.get(targetAbbreviation).getScale();
        double result = (targetScale * (value * originalRate / originalScale)) / targetRate;
        return String.format("%.2f %s = %.2f %s",
                value,
                currencyMap.get(originalAbbreviation).getCurName(),
                result,
                currencyMap.get(targetAbbreviation).getCurName());
    }
}
